...
namespace Agent.Tests
{
	...
	class AgentTest
	{
		...
		public void Null()
		{
			string hash = new Agent().Md5Hash("0");
			List<string> hashList = new List<string> { hash };
			var passArr = new Agent().SearchPassword("0", "500", hashList);

			List<KeyValuePair<string, string>> answer = new List<KeyValuePair<string, string>>();
			var a = new KeyValuePair<string, string>(hash, "0");
			answer.Add(a);
